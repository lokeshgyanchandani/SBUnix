CC=gcc
AS=as
CFLAGS=-O1 -Werror -Wall -nostdinc -Iinclude -msoft-float -mno-sse -mno-red-zone -fno-builtin -fPIC -mtune=amdfam10 -g3
LD=ld
LDLAGS=-nostdlib
AR=ar
...
submit: clean
	tar -czvf $(USER).tgz --exclude=.empty --exclude=.*.sw? --exclude=*~ LICENSE README Documentation linux_license Makefile linker.script sys bin crt libc ld include $(ROOTFS) $(USER).img
